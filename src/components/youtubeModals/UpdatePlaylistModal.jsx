...
import { getPlaylist, updatePlaylist } from '../../controllers/admin'
...
const UpdatePlaylistModal = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await updatePlaylist(currentPlaylist, title, description)
      if (res.data.success) {
        setValues({
          ...values,
          error: false,
  ...
  const loadPlaylist = async () => {
    ...
    setValues({ ...values, loading: true })
    try {
      const { data } = await getPlaylist(currentPlaylist)
      if (data.success) {
        setValues({
          ...values,
          loading: false,
  ...
  useEffect(() => loadPlaylist(), [currentPlaylist])
